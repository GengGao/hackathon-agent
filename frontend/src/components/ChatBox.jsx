	...
	return (
		<>
			...
			<div className="flex-grow p-4 overflow-y-auto scrollbar">
				...
				{messages.map((msg, i) => (
					<div
						key={`${currentSessionId || "no-session"}-msg-${i}-${msg.role}-${(msg.content || "").slice(0, 10)}`}
						className={`flex mb-4 ${
									...
									<div className="mb-3 p-3 rounded-lg bg-blue-50 border border-blue-200">
										...
										<ul className="text-xs text-blue-700 space-y-1">
											{msg.tool_calls.map((tc, idx) => (
												<li
													key={`tool-call-${idx}-${tc.name}-${(tc.arguments || "").slice(0, 10)}`}
			...
			<div className="p-4 border-t border-white/20">
				...
				>
					...
					{isTyping ? (
						...
						<button
							type="submit"
							className={`ml-3 p-2 rounded-full w-10 h-10 flex items-center justify-center transition-all duration-300 hover:scale-110 ${ragReady ? "btn-gradient" : "bg-gray-300 cursor-not-allowed"}`}
